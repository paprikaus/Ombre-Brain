...
async def api_embedding_migrate(request: Request) -> Response:
    """启动后台迁移任务：用 API 后端重算所有 bucket 的 embedding。

    Body (JSON):
        target_backend: 'api' (必填)
        api_key:        必填，写进 config.embedding.api_key
        base_url:       可选
        model:          可选（默认 gemini-embedding-001）
    ...
    target_backend = "api" if target_backend_raw in ("api", "gemini") else target_backend_raw
    if target_backend != "api":
        return JSONResponse({
            "ok": False,
            "error": f"target_backend 必须是 'api'，收到 {target_backend_raw!r}",
        }, status_code=400)
...
async def api_env_config_set(request: Request) -> Response:
    ...
    for var, val in updates.items():
        ...
        # 6. Embed 配置变更 → 同步到 embedding_engine（仅 api 后端）
        if var in ("OMBRE_EMBED_API_KEY", "OMBRE_EMBED_BASE_URL", "OMBRE_EMBED_MODEL"):
            try:
                backend_obj = getattr(embedding_engine, "_backend", None)
                if backend_obj and hasattr(backend_obj, "_client"):
                    # 已有后端：只更新对应属性
                    if var == "OMBRE_EMBED_API_KEY":
                    ...
                    elif var == "OMBRE_EMBED_BASE_URL":
                        backend_obj.base_url = value  # type: ignore[attr-defined]
                    elif var == "OMBRE_EMBED_MODEL":
                        backend_obj.model = value  # type: ignore[attr-defined]
                    # model 变更不需要重建 client；key/url 变更需要重建
                    if var in ("OMBRE_EMBED_API_KEY", "OMBRE_EMBED_BASE_URL") and value:
                        from openai import AsyncOpenAI as _OAI
                        backend_obj._client = _OAI(  # type: ignore[attr-defined]
                            api_key=getattr(backend_obj, "api_key", ""),
                            base_url=getattr(backend_obj, "base_url", ""),
                            timeout=30.0,
                        )
                elif var == "OMBRE_EMBED_API_KEY" and value:
                    # 待机模式 → 激活：用当前 env 里的 base_url/model 创建后端
                    from embedding_engine import APIEmbeddingEngine as _AEE  # type: ignore[attr-defined]
                    _base_url = (
                        os.environ.get("OMBRE_EMBED_BASE_URL", "").strip()
                        or (config.get("embedding") or {}).get("base_url", "")
                        or "https://generativelanguage.googleapis.com/v1beta/openai/"
                    )
                    _model = (
                        os.environ.get("OMBRE_EMBED_MODEL", "").strip()
                        or (config.get("embedding") or {}).get("model", "")
                        or "gemini-embedding-001"
                    )
                    new_backend = _AEE(api_key=value, base_url=_base_url, model=_model)
                    embedding_engine._backend = new_backend  # type: ignore[attr-defined]
                    embedding_engine.model = new_backend.model_name()
                    embedding_engine.enabled = True
                    embedding_engine._check_meta_consistency()
            except Exception:
                pass
