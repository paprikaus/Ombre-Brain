...
def evaluate_v3_update_manifest(manifest, content_by_path):
    ...
    if runtime is not None and hasattr(runtime, "evaluate_update_manifest"):
        return runtime.evaluate_update_manifest(manifest, content_by_path)
...
def run_v3_web_operation(
    ...
    *,
    module: str = "web",
    permissions: tuple[str, ...] = ("web:route",),
    ...
    try:
        ...
        if handler_called:
            raise
        logger.warning(f"v2.4.0 web operation wrapper failed for {module}.{operation}: {e}")
        return handler()


async def run_v3_web_async_operation(
    operation: str,
    payload: dict[str, object] | None,
    ...
    *,
    ...
    try:
        ...
        if handler_called:
            raise
        logger.warning(f"v2.4.0 async web operation wrapper failed for {module}.{operation}: {e}")
        return await handler()
